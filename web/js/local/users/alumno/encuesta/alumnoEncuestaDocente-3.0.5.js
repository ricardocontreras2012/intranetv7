function validarEncuesta() {
    ...
    const preguntasSeccion1 = [
        ...
        {
            name: "P_1_3",
            texto: "¿Con qué frecuencia cumplió Ud. con las actividades académicas y obligaciones de esta asignatura?"
        },
        {
            name: "P_1_4",
            texto: "Sin considerar las horas de clases, ¿cuántas horas a la semana, aproximadamente, dedicó Ud. a la preparación y estudio de esta asignatura?"
        }
    ];

    // Validar Sección I
    for (let i = 0; i < preguntasSeccion1.length; i++) {
    ...
    for (let j = 0; j < nombresP2.length; j++) {
        if ($('input[name="' + nombresP2[j] + '"]:checked').length === 0) {
            // Obtener texto de la pregunta desde la tabla
            const textoPregunta = $('input[name="' + nombresP2[j] + '"]').closest('tr').find('td:first').text().trim();
