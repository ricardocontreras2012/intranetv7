...
function deleteNullAttach() {
    const row = $('#attach-table tr:last').attr('id');
    if (!$("#" + row).is(":visible")) {
        if (row !== undefined) {
            $("#" + row).remove();
        }
...
function addAttach() {
    deleteNullAttach();
    const n = $("#attach-table tr").length + 1;
    const sufijo = randomString(16, '#');
...
function deleteAttach(n) {
    ...
    const l = $("#attach-table tr").length;
    for (let i = n; i <= l; i++) {
...
$(document).ready(function () {
    ...
    $('input:radio').change(function () {
        ...
        $('#messageText, #url').attr('readonly', true); // Reinicia por defecto
        $('#image-div').find('*').prop('disabled', true); // Deshabilita IMG por defecto
        ...
        switch (type) {
            ...
            case "HTML":
                ...
                break;
            case "IMG":
                $('#image-div').find('*').prop('disabled', false).first().focus(); // Enfoca primer hijo
                break;
        }
    });
