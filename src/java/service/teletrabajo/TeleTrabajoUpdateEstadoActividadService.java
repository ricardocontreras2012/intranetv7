 ...
 */
public class TeleTrabajoUpdateEstadoActividadService {
    ...
    public static String service(ActionCommonSupport action, GenericSession genericSession, TeleTrabajoSession teleSesion,
        ...
        for (int i = 0; i < teleSesion.getActividadList().size(); i++) {
            if (parameters.get("ck_" + i) != null) {
                if(teleSesion.getActividadList().get(i).getAtelEstado().equals("P"))
                {
                LogUtil.setLog(genericSession.getRut(), teleSesion.getActividadList().get(i).getId().getAtelRut());
                persistence.updateActividad(teleSesion.getActividadList().get(i).getId(), estado);
                }
            }
        }
        ...
        teleSesion.setActividadList(ContextUtil.getDAO().getActividadTeletrabajoPersistence("TT").find(genericSession.getRut(), teleSesion.getFuncionarioTeletrabajo().getFtelRut()));
