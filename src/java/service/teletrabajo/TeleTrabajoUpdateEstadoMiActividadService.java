 ...
 */
public class TeleTrabajoUpdateEstadoMiActividadService {
    ...
    public static String service(ActionCommonSupport action, GenericSession genericSession, TeleTrabajoSession teleSesion,
        ...
        for (int i = 0; i < teleSesion.getActividadList().size(); i++) {
            if (parameters.get("ck_" + i) != null) {
                if(teleSesion.getActividadList().get(i).getAtelEstado().equals("C"))
                {
        ...
        teleSesion.setActividadList(ContextUtil.getDAO().getActividadTeletrabajoPersistence("TT").findByFun(genericSession.getRut()));
