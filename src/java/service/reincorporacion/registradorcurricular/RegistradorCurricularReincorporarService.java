/*
 * @(#)RegistradorCurricularReincorporarService.java
 *
 ...
 */
public class RegistradorCurricularReincorporarService {
    ...
    public void service(GenericSession genericSession, String tipoReinc, String key) {
        ...
        try {
            ...
            Integer agno = valueOf(parametroRepo.find("agno_reincorporacion").getParValor());
            Integer sem = valueOf(parametroRepo.find("sem_reincorporacion").getParValor());
            Integer agnoReinc;
            Integer semReinc;
            Integer nomina = ContextUtil.getDAO().getScalarRepository(user).getSecuenciaNomina();

            ReincorporacionRepository reincorporacionRepo
            ...
            switch (tipoReinc) {
                case "EL": //Eliminados
                    nReinc = "1,2,3";
                    reincorporacionRepo.reincorporaEliminados(agno, sem, nomina);
                    break;
                case "MA-EL": //Masiva Eliminados
                    nReinc = "4";
                    reincorporacionRepo.reincorporaEliminadosFull(agno, sem, nomina);
                    break;
                case "RT": //Retiro Temporal
                    nReinc = "5";
                    reincorporacionRepo.reincorporaRetiros(agno, sem, nomina);
                    break;
                case "PPL": //Prorroga Periodo Lectivo
                    nReinc = "7";
                    reincorporacionRepo.reincorporaProrrogas(agno, sem, nomina);
            }
            ...
            reincorporacionList = reincorporacionRepo.find(agnoReinc, semReinc, nomina, nReinc);
    ...
    private void marcarProcesado(List<Reincorporacion> reincorporacionList) {
        ...
        for (Reincorporacion reincorporacion : reincorporacionList) {
            ...
            if (tipo >= 1 && tipo <= 3) {
                reincorporacionRepo.marcarProcesado(reincorporacion.getReiSolicitud());
            }
        }
        commitTransaction();
    }
