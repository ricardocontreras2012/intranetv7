...
import action.certificacion.alumno.AlumnoGenerarRankRegularAction;
...
import static infrastructure.util.SystemParametersUtil.C7;
 ...
 * @author Ricardo
 */
public class AlumnoGenerarRankRegularService {
 public String service(AlumnoGenerarRankRegularAction action, GenericSession genericSession, AlumnoSession as, Integer codTramite, String obs,
            Integer rank, String key) {

        WorkSession ws = genericSession.getWorkSession(key);
        String obsAux = (obs == null) ? "" : obs;

        Integer monto = CommonCertificacionUtil.getValor(as.getCertList(), C7);
        Integer folio = CommonCertificacionUtil.getFolio();
        String name = CommonCertificacionUtil.getNameFile(genericSession.getWorkSession(key).getAluCar(), folio, C7);
