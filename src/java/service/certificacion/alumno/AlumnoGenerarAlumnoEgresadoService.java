...
import action.certificacion.alumno.AlumnoGenerarAlumnoEgresadoAction;
...
import static infrastructure.util.SystemParametersUtil.C3;
 ...
 * @author Ricardo
 */
public class AlumnoGenerarAlumnoEgresadoService {

    public String service(AlumnoGenerarAlumnoEgresadoAction action, GenericSession genericSession, AlumnoSession as, Integer codTramite, String obs,
            String key) {

        WorkSession ws = genericSession.getWorkSession(key);
        String obsAux = (obs == null) ? "" : obs;

        Integer monto = CommonCertificacionUtil.getValor(as.getCertList(), C3);
        Integer folio = CommonCertificacionUtil.getFolio();
        String name = CommonCertificacionUtil.getNameFile(genericSession.getWorkSession(key).getAluCar(), folio, C3);
