package service.solicitud.inscripcion.alumno;
 ...
 *
 * Este servicio maneja el proceso de creación de una solicitud de inscripción y
 * la vinculación de cursos seleccionados por el alumno, así como los motivos de
 ...
 *
 * El flujo del servicio incluye: 1. Guardar la solicitud inicial. 2. Registrar
 * los cursos seleccionados y sus respectivos motivos. 3. Actualizar la
 * solicitud con un resumen de los cursos y motivos. 4. Registrar la acción en
 * los logs del sistema.
 *
 * @version 1.0, 24/05/2023
 */
public final class AlumnoAddSolicitudService {
     ...
     * incluyendo los motivos de inscripción y cualquier otro detalle
     * relacionado.
     *
     * El proceso se divide en varias transacciones para asegurar la integridad
     * de los datos y el registro adecuado de la información.
     *
     * @param genericSession La sesión genérica que contiene información sobre
     * la sesión del usuario.
     * @param parameters El mapa con los parámetros enviados desde la interfaz
     * de usuario.
    ...
    public String service(GenericSession genericSession, Map<String, String[]> parameters, String key) {
        ...
        parameters.entrySet().stream()
                ...
                .forEach(entry -> {
                    int row = Integer.parseInt(entry.getKey().substring(entry.getKey().lastIndexOf('_') + 1));
                    ...
                    joiner.add(curso.getNombreFull() + "->" + getMotivoDes(ws, motivo));
                });
        commitTransaction();
    ...
    /**
     * Método para obtener la descripción del motivo de inscripción a partir de
     * su código.
     *
     * Este método busca el código de motivo en la lista de motivos disponibles
     * en la sesión de trabajo y devuelve su descripción.
     *
