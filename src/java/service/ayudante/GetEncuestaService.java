 ...
 * @author Ricardo
 */
public class GetEncuestaService {
    public String service(ActionCommonSupport action, GenericSession genericSession, String key) {
        ...
        RespEnctaAyuCursoRepository respEnctaRepo
                = ContextUtil.getDAO().getRespEnctaAyuCursoRepository(ActionUtil.getDBUser());
        ComentarioEncuestaAyudanteRepository comenEnctaRepo
                = ContextUtil.getDAO().getComentarioEncuestaAyudanteRepository(ActionUtil.getDBUser());

        ws.setRespEnctaAyu(respEnctaRepo.find(curso.getId()));
        ws.setComentarioEncuestaAyudanteList(comenEnctaRepo.find(curso));
