...
/**
 * Este servicio gestiona la asignación y eliminación de docentes a un curso. 
 * Procesa los parámetros proporcionados, añadiendo o eliminando docentes según sea necesario, 
 * y luego guarda los cambios en la base de datos.
 * 
 * @author Ricardo
 */
public class CommonCursoDefinicionProfesorSaveService {
    ...
    public static String service(GenericSession genericSession, Map<String, String[]> parameters, String key) {
        ...
        for (DocenteHorario dh : ws.getDocenteHorarioList()) {
            ...
            for (Map.Entry<String, String[]> entry : parameters.entrySet()) {
                ...
                // Filtrar las entradas que corresponden a docentes (comienzan con "prof_").
                if (field.startsWith("prof_")) {
                    ...
                    String rutAux = parameters.get("prof_" + idAux)[0];
                    int rut = parseInt(rutAux.substring(0, rutAux.length() - 2));  // Eliminar el dígito verificador.
                    String horario = parameters.get("hor_" + idAux)[0];
                    ...
                    if (!"".equals(horario)) {
                        ...
                        if (dh.getProfesor().getProfRut() == rut
            ...
            if (field.startsWith("prof_")) {
                ...
                if (horario != null && !"".equals(horario)) {
                    ...
                    docenteExistente = ws.getDocenteHorarioList().stream()
                            .anyMatch(dh -> dh.getProfesor().getProfRut() == rut
                            && dh.getDhorDia() != null && dh.getDhorDia().equals(dia)
                            && dh.getDhorModulo() == modulo);
                } else {
                ...
                if (!docenteExistente) {
                    ContextUtil.getDAO().getDocenteHorarioPersistence(user).addDocente(id, rut, hor, "T");
                }
            }
        }
